...
export default function App() {
  ...
  const [introPrimed, setIntroPrimed] = useState(false); // user tapped to start
  ...
  useEffect(() => {
    if (tab === 'rounds' || tab === 'home') loadAll(true);
  }, [tab, loadAll]);
...
const handleRegister = async () => {
  ...
  if (loading) return (
    <div style={{height:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'radial-gradient(circle at 50% 30%, #0d2818 0%, #060f0a 70%)',flexDirection:'column',gap:8,overflow:'hidden',position:'relative'}}>
      ...
      <div style={{position:'relative',width:90,height:120,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end'}}>
        <div style={{animation:'wcBounce 1.1s cubic-bezier(.5,0,.5,1) infinite',width:80,height:80}}><img src='/ball.png' style={{width:80,height:80,objectFit:'contain',display:'block'}}/></div>
  ...
  // ── INTRO SCREEN ───────────────────────────────────────────────────────────
  if (showIntro) {
    // If not yet primed, show tap screen
    if (!introPrimed) return (
      <div style={{position:'fixed',inset:0,background:'radial-gradient(circle at 50% 30%, #0d2818 0%, #060f0a 70%)',zIndex:9999,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:0,padding:'0 32px'}}>
        <style>{`
          @keyframes wcBounce{0%{transform:translateY(0) rotate(0deg)}25%{transform:translateY(-40px) rotate(180deg)}50%{transform:translateY(0) rotate(360deg)}60%{transform:translateY(0) scaleX(1.15) scaleY(0.85)}100%{transform:translateY(0) rotate(540deg)}}
          @keyframes wPulse{0%,100%{opacity:.7}50%{opacity:1}}
          @keyframes wFadeIn{0%{opacity:0;transform:translateY(16px)}100%{opacity:1;transform:translateY(0)}}
          .wp-enter-btn{background:linear-gradient(135deg,#f5c518,#e6a800);color:#0a1f0d;border:none;border-radius:16px;padding:16px 48px;font-family:'Bebas Neue',sans-serif;font-size:22px;letter-spacing:3px;cursor:pointer;box-shadow:0 4px 24px rgba(245,197,24,.4);transition:transform .15s,box-shadow .15s;}
          .wp-enter-btn:active{transform:scale(.96);box-shadow:0 2px 12px rgba(245,197,24,.3);}
        `}</style>
        <div style={{animation:'wcBounce 1.2s cubic-bezier(.5,0,.5,1) infinite',marginBottom:28}}>
          <img src="/ball.png" style={{width:90,height:90,objectFit:'contain',display:'block'}}/>
        </div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:42,color:'#f5c518',letterSpacing:5,textAlign:'center',lineHeight:1,animation:'wFadeIn .6s ease both'}}>
          MUNDIAL
        </div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:42,color:'#fff',letterSpacing:5,textAlign:'center',lineHeight:1,marginBottom:6,animation:'wFadeIn .6s .1s ease both'}}>
          PORRAS
        </div>
        <img src="/trophy.png" style={{width:90,height:'auto',objectFit:'contain',marginBottom:20,animation:'wPulse 2s ease-in-out infinite'}}/>
        <div style={{fontSize:14,color:'#7da98a',textAlign:'center',lineHeight:1.6,marginBottom:32,animation:'wFadeIn .6s .2s ease both',fontWeight:500}}>
          Bienvenido al juego de porras<br/>del <span style={{color:'#f5c518',fontWeight:700}}>FIFA World Cup 2026</span> 🌍<br/>
          <span style={{fontSize:12,color:'#4d7a5a'}}>Predice, compite y gana con tus amigos</span>
        </div>
        <button className="wp-enter-btn" onClick={() => {
          setIntroPrimed(true);
        ...
        <div style={{marginTop:20,display:'flex',gap:12}}>
          <span style={{fontSize:22}}>🇪🇸</span><span style={{fontSize:22}}>🇧🇷</span><span style={{fontSize:22}}>🇩🇪</span><span style={{fontSize:22}}>🇦🇷</span><span style={{fontSize:22}}>🇫🇷</span><span style={{fontSize:22}}>🇵🇹</span>
        </div>
      </div>
    );
    return (
      <div style={{position:'fixed',inset:0,background:'#060f0a',zIndex:9999,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
        ...
        `}</style>
        <audio autoPlay src={INTRO_AUDIO} style={{display:'none'}} onCanPlay={e=>e.target.play().catch(()=>{})}/>
  ...
  const renderHome = () => {
    ...
    return (
      <>
        <div className="hero">
          <div className="ht"><Trophy size={32} style={{marginRight:10}}/>MUNDIAL PORRAS</div>
          ...
          {mySt && <div style={{marginTop:8,display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap'}}>
            <span className="pill ppt"><Trophy size={18} style={{marginRight:4}}/>{mySt.pts} pts</span>
        ...
        {players.length === 0 && !currentPlayer && (
          <div className="card" style={{textAlign:'center',padding:'28px 20px'}}>
            ...
            <button className="btn" onClick={() => setModal('register')}><Ball size={22} style={{marginRight:8}}/>Registrarme</button>
          </div>
        )}
        ...
        {nextRound && (
          <>
            ...
            <div className="card">
              ...
              {canBet(nextRound) && <button className="btn" style={{marginTop:12}} onClick={() => { setSelectedRoundId(nextRound.id); setBetDraft({}); setModal('bet'); }}><Ball size={22} style={{marginRight:8}}/>Hacer mi porra</button>}
              {hasBet(nextRound) && <div className="prsent">✅ Porra enviada — ¡Buena suerte!</div>}
              {!currentPlayer && <button className="btn" style={{marginTop:12}} onClick={() => setModal('register')}><Ball size={22} style={{marginRight:8}}/>Unirse</button>}
            </div>
          </>
        )}
        ...
        {!currentPlayer && <button className="btn" style={{marginTop:8}} onClick={() => setModal('register')}><Ball size={22} style={{marginRight:8}}/>Unirse al mundial</button>}
      </>
    );
  };
  ...
  const renderStandings = () => (
    <>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div className="ct" style={{fontSize:22}}><Trophy size={30} style={{marginRight:10}}/>CLASIFICACIÓN</div>
        ...
        {[...rounds].reverse().map(r => {
          ...
          return (
            <div key={r.id} className="card">
              ...
              {currentPlayer && hasBet(r) && r.results_entered && <div style={{textAlign:'center',fontSize:12,color:'var(--text-dim)',marginTop:4}}>✅ Participaste en esta jornada</div>}
            </div>
          );
        })}
      </>
  ...
  const renderProfile = () => {
    ...
    return (
      <>
        ...
        <div className="sg">
          {[{l:'Puntos',v:st?.pts||0,i:<Trophy size={28}/>},{l:'Exactos',v:st?.exact||0,i:'🎯'},{l:'Signos',v:st?.sign||0,i:'✅'},
        ...
        {myHistory.length > 0 && (
          <>
            ...
            <div className="card" style={{padding:'12px 16px',overflowX:'auto'}}>
              <div style={{display:'flex',alignItems:'flex-end',gap:6,height:70,minWidth:0,width:'100%'}}>
                {myHistory.slice(-10).map((h,i) => {
                  const maxH = Math.max(...myHistory.map(x=>x.pts), 1);
                  const isMax = h.pts === Math.max(...myHistory.map(x=>x.pts));
                  return (
                    <div key={i} style={{flex:'1 1 0',minWidth:28,maxWidth:52,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                      <div style={{fontSize:9,color:'var(--gold)',fontWeight:700}}>{h.pts}</div>
                      <div style={{width:'100%',height:`${Math.max(8,(h.pts/maxH)*50)}px`,background:isMax?'var(--gold)':'var(--pitch-light)',borderRadius:'4px 4px 0 0',transition:'height .3s'}}/>
                      <div style={{fontSize:8,color:'var(--text-muted)',textAlign:'center',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:40}}>{h.roundName.replace('Jornada ','J').replace('Eliminatorias','Elim').replace('Grupo','G')}</div>
                    </div>
                  );
                })}
              </div>
              {myHistory.length > 10 && <div style={{fontSize:10,color:'var(--text-muted)',textAlign:'center',marginTop:6}}>Mostrando últimas 10 jornadas</div>}
            </div>
          </>
        )}
          ...
          <>
            ...
            {rounds.filter(r=>!r.results_entered).map(r => (
              <div key={r.id} className="adms">
                ...
                <button className="btn" onClick={() => handleEnterResults(r.id)}><Trophy size={22} style={{marginRight:8}}/>Guardar resultados de {r.name}</button>
              </div>
            ))}
          </>
        )}
  ...
  const renderBetModal = () => {
    ...
    return (
      <div className="ov" onClick={e => e.target===e.currentTarget && setModal(null)}>
        <div className="modal">
          <div className="mhdl"/>
          <div className="mtit"><Ball size={26} style={{marginRight:8}}/>{round.name} — Tu porra</div>
    ...
    <>
      ...
      <div className="app">
        <div className="topbar">
          <div className="logo">MUNDIAL<span> PORRAS</span> <Trophy size={28} style={{marginLeft:6}}/></div>
